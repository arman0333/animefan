...
module_data = path.join(path.dirname(__file__), "content")
ascii_path = path.join(module_data, "ascii-images")
...
def build_bubble(quote=""):
    ...
    for i in range(len(wrap_list)):
       length_diff = wrap_limit - len(wrap_list[i])
       wrap_list += (' ' * length_diff) + '|'
